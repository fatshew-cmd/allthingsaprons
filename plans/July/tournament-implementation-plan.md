...
**Status (as of 2026-07-05): Phase 1 done, Phase 2 done and substantially extended beyond spec (edit wizard, jury invite/accept/decline, jury replace, self-cancel), Phase 3 now substantially reworked (candidate submission moved to upload-time, no more post-hoc "pick an entry" picker; organizer approve/reject now run on a rolling basis across both `open` and `cooldown`, not cooldown-only — see "Phase 3 Extensions" below; group/knockout scheduling is still not built), Phase 9 partially done and includes a full tournament social layer (comments, watch/subscribe, tournament-level report), a redesigned browse list (index.ejs), and stain-tag browsing/wildcard auto-draft. Phases 4–8 (group assignment through prize payout) not started — see the gap called out below.**
