...
- **Phase 4 done as of 2026-07-05 (later); Phases 5 and 6 done as of 2026-07-06 (see below); Phases 7–8 not started.** `utils/tournamentScheduler.js` now exists (140 lines, exports `generateGroups`, `generateGroupMatches`, `circleMethodRounds`) and is fully wired: `activateTournament` (in `jobs/tournamentJobs.js`) atomically flips `cooldown → active`, stamps `stage: 'group'`, then calls `generateGroups` (Fisher-Yates shuffle, chunked into `groupSize`-sized `TournamentGroup` docs, back-filling `groupId` onto each member's `TournamentEntry`) followed by `generateGroupMatches` per group — a standard "circle method" round-robin (diverges from this doc's original greedy slot-scheduler draft in 4C below, but delivers the same guarantee: nobody double-booked in a round, in the minimum possible number of rounds). The first round's `Contest`/`TournamentMatch` docs are created `active` immediately; later rounds are `scheduled` and opened by a new agenda job, `open_tournament_match` (not in the original Phase 4 draft), which flips them active and gives the `Contest` a fresh 24h `votingDeadline` — relying on the existing 15-min contest sweeper to close it, rather than explicitly scheduling `close_contest` the way 4C's draft assumed. `activateTournament` is reachable two ways: the `tournament_cooldown_expiry` agenda job, and the organizer-triggered `POST /api/tournaments/:id/advance-now` (this doc's draft 4E assumed a `/launch` route reachable only from `cooldown`; the real route is named `advance-now`, lives in `routes/api.js`, and works from `open` or `cooldown` alike — see "Phase 3 Extensions" above).
  ...
  **Phase 9I — As-Built (2026-07-06/7, uncommitted).** `views/admin/tournaments/index.ejs` and `review.ejs` — previously static skeletons reflecting the old superseded `pending_review` admin-review design — have been repurposed per this section's own spec below, not left as placeholders. `GET /admin/tournaments` (`routes/admin.js`) now runs a real query: status filter (`open|cooldown|active|closed|canceled`), organizer populate, prize pool, and an approved-participant-count aggregation, capped at 200 results sorted newest-first (not paginated, and not filterable by type — there's only ever one tournament type in the live schema). `review.ejs` was deleted rather than repurposed — the spec below assumed a `pending_review` state gating user-organized tournaments pre-launch, but that state never made it into the real `Tournament.status` enum (`open|cooldown|active|closed|canceled`), so there's nothing to review-queue; any organizer clearing `requireOrganizerEligibility`'s automated checks goes live immediately with no human review step at all. In its place: a new `GET /admin/tournaments/:id` (`views/admin/tournaments/detail.ejs`) renders full read-only state — groups (with members), matches, jury count, approved count, organizer details — plus a `POST /admin/tournaments/:id/cancel` "Force Cancel" action (shown only for `open`/`cooldown`) that calls the existing `cancelTournament` job helper and logs a new `tournament_force_canceled` audit event type. The sidebar's "Review Queue" link was removed alongside `review.ejs`.
  ...
  The only real gaps left anywhere in the tournament feature, verified against the code rather than assumed from this doc: no admin moderation queue reads `TournamentCommentReport`/`TournamentReport` yet (reports land in the DB with no moderator-facing surface — see "Phase 9 Extensions" above); `views/tournaments/index.ejs` browse cards are uniform across tabs, missing the per-tab spots-filled/countdown/progress/winner details from 9B below; and the "Follow-up Tasks" section's organizer notification opt-out toggle remains unscheduled. None of the Phase 7 Group-Tie Extension, the two notification/viewCount fixes, or Phase 9I's admin pages are committed yet — last commit is still `dc69249`.

Next step: the tournament comment/report moderation queue, and the browse-page per-tab card details — everything else in the tournament feature (backend gameplay/payout loop, tie resolution at both match and group-ranking level, and the frontend/admin surface) is complete.
