...
- **Phase 4 done as of 2026-07-05 (later); Phases 5 and 6 done as of 2026-07-06 (see below); Phases 7–8 not started.** `utils/tournamentScheduler.js` now exists (140 lines, exports `generateGroups`, `generateGroupMatches`, `circleMethodRounds`) and is fully wired: `activateTournament` (in `jobs/tournamentJobs.js`) atomically flips `cooldown → active`, stamps `stage: 'group'`, then calls `generateGroups` (Fisher-Yates shuffle, chunked into `groupSize`-sized `TournamentGroup` docs, back-filling `groupId` onto each member's `TournamentEntry`) followed by `generateGroupMatches` per group — a standard "circle method" round-robin (diverges from this doc's original greedy slot-scheduler draft in 4C below, but delivers the same guarantee: nobody double-booked in a round, in the minimum possible number of rounds). The first round's `Contest`/`TournamentMatch` docs are created `active` immediately; later rounds are `scheduled` and opened by a new agenda job, `open_tournament_match` (not in the original Phase 4 draft), which flips them active and gives the `Contest` a fresh 24h `votingDeadline` — relying on the existing 15-min contest sweeper to close it, rather than explicitly scheduling `close_contest` the way 4C's draft assumed. `activateTournament` is reachable two ways: the `tournament_cooldown_expiry` agenda job, and the organizer-triggered `POST /api/tournaments/:id/advance-now` (this doc's draft 4E assumed a `/launch` route reachable only from `cooldown`; the real route is named `advance-now`, lives in `routes/api.js`, and works from `open` or `cooldown` alike — see "Phase 3 Extensions" above).
  ...
  **Phase 8 — As-Built (verified 2026-07-06 against the working tree, which was ahead of this doc).** `closeTournament(tournamentId)` (`jobs/tournamentJobs.js`) is called from `handleKnockoutMatchClose` whenever a `Final` or `3rd`-place match closes (they can close in either order — whichever call actually finds both closed proceeds; the other returns early). It re-fetches both matches, derives placements (final winner = 1st, final loser = 2nd, 3rd-place winner = 3rd; a 4-player tournament has no 3rd-place match and skips that placement entirely), and atomically claims `status: { $ne: 'closed' } → 'closed'` so only one of the (possibly simultaneous) callers pays out. It credits each placement's prize into the winner's `earnedCHL` via `creditWallet` (isolated in a per-placement try/catch so one failed credit can't strand the rest), fires a `tournament_prize_awarded` notification per winner and a `tournament_closed` notification to every approved participant, sets `prizes.winnersSet: true`, and finally bans every `TournamentJury` member with `missedVotes > 0` by setting `User.juryBanned = true`. This matches the 8A draft below almost exactly, including the jury-ban step from 7G. **8B (tournament sweeper) is also built** — `jobs/sweeper.js`'s `runTournamentSweeper` covers all four one-time-job types (open/cooldown/jury-vote-expiry/organizer-vote-expiry) as a crash-recovery net, plus an additional `runTournamentMatchReconcileSweeper` (not in the original 8B draft) that re-fires `handleTournamentMatchClose` for any `TournamentMatch` left stranded by a process crash between a `Contest` closing and the tournament-side handler finishing.
  ...
  The only real gaps left anywhere in the tournament feature, verified against the code rather than assumed from this doc: no bracket/tree visualization on `detail.ejs` (matches still render as one flat list); `tournament_knockout_started` was added to `Notification`'s enum and is fired from `utils/tournamentScheduler.js`, but was never given a display case in `views/notifications.ejs`; `Tournament.viewCount` exists and displays but nothing increments it; `views/admin/tournaments/index.ejs` and `review.ejs` are still static "coming soon"-style placeholders wired to routes that render them with no query logic at all; and a 3+-way group-ranking boundary tie is explicitly left unresolved (`resolveGroup` logs a warning and returns) since the jury system only ever breaks a *match* tie, not a group-ranking tie.

Next step: the frontend bracket/tree visualization, the missing `tournament_knockout_started` notification case, the `viewCount` increment, and the admin tournament pages — the backend gameplay/payout loop itself is complete.
