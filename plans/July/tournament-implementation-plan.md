...
The `views/tournaments/index.ejs` browse-card gap the simulation pass left unchanged (uniform cards across tabs, no per-tab spots-filled/countdown/progress/winner details) was closed 2026-07-12 — see "browse-card progress details" below. The only genuine gap left anywhere in the feature is an unscheduled follow-up task for an organizer notification opt-out toggle (see "Follow-up Tasks"). Testing categories the simulation harness deliberately doesn't cover — pixel/visual browser confirmation, adversarial/malformed-input fuzzing, load/stress at scale, and multi-process concurrency — are noted as out-of-scope-for-this-tool in that report's §21, not suspected gaps.
...
- **Phase 4 done as of 2026-07-05 (later); Phases 5 and 6 done as of 2026-07-06 (see below); Phases 7–8 not started.** `utils/tournamentScheduler.js` now exists (140 lines, exports `generateGroups`, `generateGroupMatches`, `circleMethodRounds`) and is fully wired: `activateTournament` (in `jobs/tournamentJobs.js`) atomically flips `cooldown → active`, stamps `stage: 'group'`, then calls `generateGroups` (Fisher-Yates shuffle, chunked into `groupSize`-sized `TournamentGroup` docs, back-filling `groupId` onto each member's `TournamentEntry`) followed by `generateGroupMatches` per group — a standard "circle method" round-robin (diverges from this doc's original greedy slot-scheduler draft in 4C below, but delivers the same guarantee: nobody double-booked in a round, in the minimum possible number of rounds). The first round's `Contest`/`TournamentMatch` docs are created `active` immediately; later rounds are `scheduled` and opened by a new agenda job, `open_tournament_match` (not in the original Phase 4 draft), which flips them active and gives the `Contest` a fresh 24h `votingDeadline` — relying on the existing 15-min contest sweeper to close it, rather than explicitly scheduling `close_contest` the way 4C's draft assumed. `activateTournament` is reachable two ways: the `tournament_cooldown_expiry` agenda job, and the organizer-triggered `POST /api/tournaments/:id/advance-now` (this doc's draft 4E assumed a `/launch` route reachable only from `cooldown`; the real route is named `advance-now`, lives in `routes/api.js`, and works from `open` or `cooldown` alike — see "Phase 3 Extensions" above).
  ...
  **`views/tournaments/index.ejs` browse-card progress details — built 2026-07-12.** `routes/tournaments.js`'s `GET /tournaments` now calls three batched helpers (`attachSpotsFilled`, `attachProgressLabel`, `attachWinners`) after fetching the live/closed lists: open/cooldown cards show `spotsFilled/size` (candidates submitted while `open`, approved count while `cooldown`); active cards show a real progress label (`Group Stage · X/Y played`, or the current knockout round name — `3rd` and `Final` run concurrently so both collapse to "Final"); closed cards show the Final match winner's avatar + name via a batched version of the existing `loadTournamentPlacements` logic. All three are single aggregate/find queries across the whole visible list, not per-card N+1 lookups.

  The only item left anywhere in the tournament feature, verified against the code rather than assumed from this doc: the "Follow-up Tasks" section's organizer notification opt-out toggle remains unscheduled — a minor, optional nice-to-have, not a gap in shipped functionality. Everything else — backend gameplay/payout loop, tie resolution at both match and group-ranking level, the full frontend/admin surface including comment/report moderation, and now the browse-card progress details — is complete and simulation-verified as of 2026-07-12.
