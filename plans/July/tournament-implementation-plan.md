...
> **This is the sole authoritative source for tournament phase numbering (Phases 1–9 below cover the entire feature, schema through frontend/admin/notifications).** `july-action-plan.md`'s original Phase 7/8 tournament draft has been fully collapsed into a short pointer at that doc, and its unrelated Phase 9 (Financial System)/Phase 10 (Polish) were renumbered to Phase 10/11 specifically to stop colliding with this doc's Phase 9/8. Do not track tournament status by phase number anywhere but here.
> Spec source: `plans/July/tournament-spec.md`.
> Written against actual codebase structure as of 2026-07-02.

**Status (as of 2026-07-09, committed): Phases 1–9 are complete and simulation-verified.** Schema, creation flow, candidate management, group scheduling, group-stage advancement (including 3+-way group-ranking ties), knockout bracket generation + progression, jury/organizer/coin-flip tie resolution (match-level and group-ranking-level alike), prize payout, and the full Phase 9 frontend/admin/notification surface — a real knockout-bracket visualization on `detail.ejs` (grouped by round — R16/QF/SF/3rd/Final — with winner highlighting); `tournament_knockout_started`'s display case in `views/notifications.ejs`; `Tournament.viewCount` incrementing on every `GET /tournament/:id`; real `views/admin/tournaments/index.ejs`/`detail.ejs` queries plus "Force Cancel" (see "Phase 9I — As-Built" below); a 3+-way group-ranking boundary tie running the identical 6h-jury → 3h-organizer → coin-flip chain a match tie does (see "Phase 7 — Group-Tie Extension" below); and the admin moderation queue for `TournamentCommentReport`/`TournamentReport` (`views/admin/moderation.ejs`'s "Tournaments" tab) — are all live and committed. Nothing tournament-related is sitting uncommitted as of the 2026-07-09 auto-commit.

**Simulation pass (2026-07-08, `plans/July/tournament-simulation-report.md`, harness `scripts/simulateTournament.js`):** a 15-scenario end-to-end pass drove the entire lifecycle — creation through prize payout, every tie-resolution branch (2-way and 3+-way group ties; match-level ties; jury-quorum, organizer-decision, and coin-flip resolution paths), all five valid tournament sizes (4/8/12/16/24, including the R16-deep 24-player bracket), the organizer/jury H2H-voting restriction, open-phase and cooldown edge cases, the full social/admin layer, and genuinely concurrent match closes — through real HTTP requests against the running dev server, not just a code read. It confirmed the backend gameplay loop and Phase 9 frontend/admin surface actually work end-to-end, and found + fixed two real product bugs (both now committed):
- A juror who **declined** their jury invite stayed incorrectly barred from voting in unrelated standalone H2H contests — `routes/api.js`'s org/jury voting-restriction check (`TournamentJury.distinct('tournamentId', {...})`) was missing a `status: 'accepted'` filter, so a mere invite record (regardless of response) triggered the block. Fixed by adding `status: 'accepted'` to the query, matching every other jury-facing check in `jobs/tournamentJobs.js`.
- **The most serious tournament bug found to date:** approving a `TournamentEntry` whose underlying `Entry` had been deleted (self-delete, moderation removal, etc.) succeeded silently with no validation. If that orphaned approval survived to cooldown expiry, `generateGroupMatches()` (`utils/tournamentScheduler.js`) crashed on a null dereference (`A.entryId._id` where a deleted `Entry` populates to `null`) — but only *after* `activateTournament()`'s atomic `cooldown → active` status claim had already committed, **permanently wedging the tournament** with no possible retry (nothing re-enters activation from `status: 'active'`). Fixed at the root cause: the candidate-approve route (`routes/api.js`) now checks `Entry.exists({_id: entry.entryId})` and rejects with `409` if the underlying entry is gone; rejecting (not approving) an orphaned candidate remains the organizer's normal path to clear it, and now correctly shrinks the pool below the cap instead of time-bombing the tournament.

The remaining genuine gaps are unchanged by the simulation pass: `views/tournaments/index.ejs` browse cards are uniform across tabs (no per-tab spots-filled/countdown/progress/winner details, per 9B below); and an unscheduled follow-up task exists for an organizer notification opt-out toggle (see "Follow-up Tasks"). Testing categories the simulation harness deliberately doesn't cover — pixel/visual browser confirmation, adversarial/malformed-input fuzzing, load/stress at scale, and multi-process concurrency — are noted as out-of-scope-for-this-tool in that report's §21, not suspected gaps.
...
- **Phase 4 done as of 2026-07-05 (later); Phases 5 and 6 done as of 2026-07-06 (see below); Phases 7–8 not started.** `utils/tournamentScheduler.js` now exists (140 lines, exports `generateGroups`, `generateGroupMatches`, `circleMethodRounds`) and is fully wired: `activateTournament` (in `jobs/tournamentJobs.js`) atomically flips `cooldown → active`, stamps `stage: 'group'`, then calls `generateGroups` (Fisher-Yates shuffle, chunked into `groupSize`-sized `TournamentGroup` docs, back-filling `groupId` onto each member's `TournamentEntry`) followed by `generateGroupMatches` per group — a standard "circle method" round-robin (diverges from this doc's original greedy slot-scheduler draft in 4C below, but delivers the same guarantee: nobody double-booked in a round, in the minimum possible number of rounds). The first round's `Contest`/`TournamentMatch` docs are created `active` immediately; later rounds are `scheduled` and opened by a new agenda job, `open_tournament_match` (not in the original Phase 4 draft), which flips them active and gives the `Contest` a fresh 24h `votingDeadline` — relying on the existing 15-min contest sweeper to close it, rather than explicitly scheduling `close_contest` the way 4C's draft assumed. `activateTournament` is reachable two ways: the `tournament_cooldown_expiry` agenda job, and the organizer-triggered `POST /api/tournaments/:id/advance-now` (this doc's draft 4E assumed a `/launch` route reachable only from `cooldown`; the real route is named `advance-now`, lives in `routes/api.js`, and works from `open` or `cooldown` alike — see "Phase 3 Extensions" above).
  ...
  **Phase 7 — Group-Tie Extension (2026-07-06/7, uncommitted).** The unresolvable-boundary-tie warning above is now stale — a 3+-way group-ranking tie runs the identical 6h-jury → 3h-organizer → coin-flip chain a match tie does, capped at 9h total, rather than being left flagged. New model `TournamentGroupTieVote` (one top-pick vote per juror, unique on `{groupId, jurorId}`); new `tieStatus`/`tieDeadline`/`tiedEntryIds`/`tieSlotsForCluster`/`tieResolvedOrder` fields on `TournamentGroup` mirroring `TournamentMatch`'s tie fields. `resolveGroup` (`jobs/tournamentJobs.js`) now branches: a fresh 3+-way tie calls `resolveGroupTieCluster` → `initiateGroupTieResolution` (notifies jurors, 6h window); quorum-3 jury votes resolve via `resolveGroupJuryVote` unless the plurality itself leaves the advance/eliminate cutoff ambiguous, in which case `tournamentGroupJuryExpiry` hands off to the organizer's 3h window (`resolveGroupOrganizerVote`) and finally `tournamentGroupOrganizerVoteExpiry`'s coin flip. Resolution is anchored to the tie's stored snapshot rather than recomputed live, so rating drift during the 6-9h window can't strand the group. New routes `POST /api/tournaments/:id/groups/:groupId/jury-vote` and `.../organizer-vote`; new pages `GET /tournament/:id/group-jury-vote/:groupId` and `.../group-organizer-vote/:groupId` (views `group-jury-vote.ejs`/`group-organizer-vote.ejs`, sharing a new `views/partials/groupTieBreakPicker.ejs`, blind-judged the same way as `tieBreakPicker.ejs`). Two new notification types (`tournament_group_tie_jury`, `tournament_group_tie_organizer`) have display cases in `notifications.ejs`. `jobs/sweeper.js` got matching crash-recovery sweeps for both new deadline fields.

  Also fixed in the same uncommitted pass: `tournament_knockout_started` now has a display case in `views/notifications.ejs` (previously fell through to the generic default), and `Tournament.viewCount` is now incremented in `GET /tournament/:id` (`routes/tournaments.js`) — it had been removed since 2026-07-04 with an open "restore or drop the display" question, now resolved by restoring it.

  `routes/tournaments.js`'s `loadTournamentPlacements` helper and `routes/pages.js`'s profile trophy counters (`firstPrizes`/`secondPrizes`/`thirdPrizes`) are no longer dead code as of Phase 8 shipping — `Tournament.status` reaches `'closed'` now, so both are live.

  **Phase 8 — As-Built (verified 2026-07-06 against the working tree, which was ahead of this doc).** `closeTournament(tournamentId)` (`jobs/tournamentJobs.js`) is called from `handleKnockoutMatchClose` whenever a `Final` or `3rd`-place match closes (they can close in either order — whichever call actually finds both closed proceeds; the other returns early). It re-fetches both matches, derives placements (final winner = 1st, final loser = 2nd, 3rd-place winner = 3rd; a 4-player tournament has no 3rd-place match and skips that placement entirely), and atomically claims `status: { $ne: 'closed' } → 'closed'` so only one of the (possibly simultaneous) callers pays out. It credits each placement's prize into the winner's `earnedCHL` via `creditWallet` (isolated in a per-placement try/catch so one failed credit can't strand the rest), fires a `tournament_prize_awarded` notification per winner and a `tournament_closed` notification to every approved participant, sets `prizes.winnersSet: true`, and finally bans every `TournamentJury` member with `missedVotes > 0` by setting `User.juryBanned = true`. This matches the 8A draft below almost exactly, including the jury-ban step from 7G. **8B (tournament sweeper) is also built** — `jobs/sweeper.js`'s `runTournamentSweeper` covers all four one-time-job types (open/cooldown/jury-vote-expiry/organizer-vote-expiry) as a crash-recovery net, plus an additional `runTournamentMatchReconcileSweeper` (not in the original 8B draft) that re-fires `handleTournamentMatchClose` for any `TournamentMatch` left stranded by a process crash between a `Contest` closing and the tournament-side handler finishing.

  **Phase 9I — As-Built (2026-07-06/7, uncommitted).** `views/admin/tournaments/index.ejs` and `review.ejs` — previously static skeletons reflecting the old superseded `pending_review` admin-review design — have been repurposed per this section's own spec below, not left as placeholders. `GET /admin/tournaments` (`routes/admin.js`) now runs a real query: status filter (`open|cooldown|active|closed|canceled`), organizer populate, prize pool, and an approved-participant-count aggregation, capped at 200 results sorted newest-first (not paginated, and not filterable by type — there's only ever one tournament type in the live schema). `review.ejs` was deleted rather than repurposed — the spec below assumed a `pending_review` state gating user-organized tournaments pre-launch, but that state never made it into the real `Tournament.status` enum (`open|cooldown|active|closed|canceled`), so there's nothing to review-queue; any organizer clearing `requireOrganizerEligibility`'s automated checks goes live immediately with no human review step at all. In its place: a new `GET /admin/tournaments/:id` (`views/admin/tournaments/detail.ejs`) renders full read-only state — groups (with members), matches, jury count, approved count, organizer details — plus a `POST /admin/tournaments/:id/cancel` "Force Cancel" action (shown only for `open`/`cooldown`) that calls the existing `cancelTournament` job helper and logs a new `tournament_force_canceled` audit event type. The sidebar's "Review Queue" link was removed alongside `review.ejs`.

  The only real gaps left anywhere in the tournament feature, verified against the code rather than assumed from this doc: `views/tournaments/index.ejs` browse cards are uniform across tabs, missing the per-tab spots-filled/countdown/progress/winner details from 9B below; and the "Follow-up Tasks" section's organizer notification opt-out toggle remains unscheduled. The admin moderation queue for `TournamentCommentReport`/`TournamentReport` (previously listed here as a gap) is confirmed live as of 2026-07-07 — see "Phase 9 Extensions" above. The Phase 7 Group-Tie Extension, the two notification/viewCount fixes, and Phase 9I's admin pages are all committed as of the 2026-07-09 auto-commit, and the entire feature has since been driven end-to-end by the 2026-07-08 simulation pass (see the status header above) — including finding and fixing the two product bugs described there.

Next step: the browse-page per-tab card details — everything else in the tournament feature (backend gameplay/payout loop, tie resolution at both match and group-ranking level, and the frontend/admin surface, including comment/report moderation) is complete and simulation-verified.
...
views/admin/tournaments/ — index.ejs + detail.ejs (real queries, done 2026-07-06/7; review.ejs deleted, see Phase 9I)
scripts/simulateTournament.js — end-to-end lifecycle simulation harness (2026-07-08), see plans/July/tournament-simulation-report.md
```
...
## Phase 8 — Prize Distribution & Tournament Close ✅ done 2026-07-06 — see "Phase 8 — As-Built" note in the status header above

### 8A. `closeTournament(tournamentId)` in `jobs/tournamentJobs.js` — built essentially as drafted below
...
### 8B. Tournament sweeper (add to `jobs/sweeper.js`) — built, plus an extra crash-recovery reconcile sweeper not drafted here (see "Phase 8 — As-Built" note above)
...
### Phase 8 exit criteria — all met, verified 2026-07-06
...
- `WalletTransaction` records exist for each payout (via `creditWallet`)
...
- Jury members with missed votes are permanently flagged (`User.juryBanned`)
...
`TournamentLoop`/`TournamentEntryLoop` are mounted only in `routes/tournaments.js`/`routes/api.js`. `TournamentCommentReport`/`TournamentReport`, however, **are** surfaced to moderators (verified 2026-07-07, corrects the earlier note in this doc): `views/admin/moderation.ejs` has a dedicated "Tournaments" tab (badge-counted, empty state, `POST /admin/moderation/tournament-reports/:tid/dismiss`) and tournament comment reports are merged into the general comment-report queue with approve/reject actions, both backed by `routes/admin.js`.
...
### 9I. Admin pages ✅ done 2026-07-06/7, uncommitted — see "Phase 9I — As-Built" note above

**Update `views/admin/tournaments/index.ejs`** — done, close to spec:
- Table-ish list: Tournament Name | Organizer | Size | Prize Pool | Status (no separate "Created At"/"Actions" columns, but each row links to detail)
- Filter by status (open / cooldown / active / closed / canceled) — done
- Each row links to detail view — done
- No review/approve/reject actions (admin review step was removed) — correct, done

**`views/admin/tournaments/review.ejs` — deleted, not repurposed** (diverges from spec below): the read-only detail view was built fresh as a new `views/admin/tournaments/detail.ejs` instead of reusing `review.ejs`'s file, since `review.ejs`'s whole premise (a `pending_review`-gated queue) never existed in the shipped `Tournament.status` enum:
- Shows full tournament state: groups, matches, jury member count (not identities), prize pool — done (no standings table specifically, but groups/matches cover it)
- "Force Cancel" button — done, but gated on `requireDomain('tournaments')`/tournament-status (`open`/`cooldown` only) rather than founder-only as drafted below; see the route spec immediately below, which itself says `requireDomain(null)` (founder) for the cancel action specifically — **the shipped route matches that founder-only gate**, it's just the *page-level* view gate that uses `requireDomain('tournaments')`
...
**Update 2026-07-06 — verified against `views/notifications.ejs`: all 8 of the below now render, as Phases 4–8 landed.** The only tournament notification type left without a display case is `tournament_knockout_started` (added to the enum from Phase 6, fired from `utils/tournamentScheduler.js`, but never given a `case` in `views/notifications.ejs` — falls through to the generic default). That's the one remaining gap here, not a whole batch of 8.
```
'tournament_live'                 — case exists (line ~130)
'tournament_group_advance'        — case exists (line ~138)
'tournament_eliminated'           — case exists (line ~140)
'tournament_tie_jury'             — case exists (line ~142)
'tournament_tie_organizer'        — case exists (line ~144)
'tournament_jury_vote_received'   — case exists (line ~146)
'tournament_closed'               — case exists (line ~148)
'tournament_prize_awarded'        — case exists (line ~150)
```
(`tournament_cooldown_started`, `tournament_canceled`, `tournament_entry_submitted`, `tournament_entry_approved`, `tournament_entry_rejected` already existed per above — 8 + 5 = the original 13, all done. `tournament_entry_match_live`/`tournament_entry_match_closed`/`tournament_knockout_started` were added beyond the original 13; the first two have display cases, `tournament_knockout_started` does not — see the gap noted in the status header.)
...
### Phase 9 exit criteria — all met as of 2026-07-06/7 (uncommitted)

- Full tournament lifecycle is navigable via browser from creation to close ✅
- All 5 view files render correctly at each lifecycle state ✅ (plus several more not in the original 5: group/organizer tie-vote pages, jury-invite/manage, admin detail)
- Jury vote page reveals no user identity information ✅ (both the match-tie and group-tie pickers are blind-judged)
- Right panel shows active tournaments for all logged-in users ✅
- Admin can browse all tournaments and view full detail (read-only) ✅ (`/admin/tournaments` + `/admin/tournaments/:id`, plus a Force Cancel action beyond "read-only")
- All notification types render correctly in `/notifications` ✅ (grew well beyond the original 13 — see 9J below; the last gap, `tournament_knockout_started`, was fixed 2026-07-06/7)
...
- Surface `TournamentCommentReport`/`TournamentReport` in the admin moderation queue — both models exist and get written to (a comment report auto-hides the comment immediately), but nothing in `/admin/moderation` reads either collection, so reports currently just sit in the DB with no moderator-facing surface at all.
- Build out the per-tab progress details on `views/tournaments/index.ejs` browse cards that 9B originally specced and never got: spots-filled count, days-remaining countdown, group/knockout progress indicator, winner username+avatar on closed-tab cards. Cards are currently uniform across all tabs.
