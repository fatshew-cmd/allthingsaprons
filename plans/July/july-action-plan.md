...
## Current State (July 12 — committed)

`views/tournaments/index.ejs` browse cards picked up the per-tab progress details flagged as the last real gap in the entry below: `routes/tournaments.js`'s `GET /tournaments` now runs three batched helpers (`attachSpotsFilled`, `attachProgressLabel`, `attachWinners`) after fetching the live/closed lists. Open/cooldown cards show `spotsFilled/size` (candidates submitted while `open`, approved count while `cooldown`); active cards show a real progress label (`Group Stage · X/Y played`, or the current knockout round name, with `3rd`/`Final` collapsing to "Final"); closed cards show the Final match winner's avatar + name via a batched version of the existing `loadTournamentPlacements` logic. All three are single aggregate/find queries across the whole visible list, not per-card N+1 lookups. Full breakdown in `tournament-implementation-plan.md`'s status header.

With this shipped, the only tracked gap left anywhere in the tournament feature is the unscheduled, optional "Follow-up Tasks" item for an organizer notification opt-out toggle — not a gap in shipped functionality.
...
**Status summary (2026-07-12, committed, simulation-verified):** the entire backend tournament gameplay loop is complete per `tournament-implementation-plan.md`'s Phases 1–8 — creation/funding, candidate submission + organizer review, group-stage play + both 2-way and 3+-way tie resolution, knockout bracket generation + progression, jury/organizer/coin-flip tie resolution (match-level and group-ranking-level alike), and prize payout. That doc's Phase 9 (frontend/admin/notifications) is also done: browse/detail/create views (including real per-tab progress details on browse cards as of 2026-07-12 — spots-filled, group/knockout progress, closed-tab winner), right panel wiring, the full social layer (comments/loop-in/report), a real knockout bracket visualization on `detail.ejs`, and `/admin/tournaments` (list + detail + force-cancel) are all live. The admin moderation queue for `TournamentCommentReport`/`TournamentReport` is also live (verified 2026-07-07): `views/admin/moderation.ejs` has a dedicated "Tournaments" tab (badge-counted, empty state, dismiss action) plus tournament comment reports merged into the general comment-report queue, backed by `routes/admin.js` (`POST /admin/moderation/tournament-reports/:tid/dismiss` and the tournament-comment approve/reject routes). A 15-scenario end-to-end simulation pass (2026-07-08, `tournament-simulation-report.md`) drove this entire loop through real HTTP requests and found + fixed two real product bugs (a declined juror wrongly still barred from H2H voting; a far more serious permanent-wedge crash from approving a candidate whose `Entry` had been deleted) — see the "Current State (July 8/9)" entry above for details. The only tracked gap left anywhere in the feature is an unscheduled, optional follow-up task for an organizer notification opt-out toggle. There is also no pre-launch admin review/approval gate for user-organized tournaments — the `pending_review` status from the original draft below was never implemented, so any organizer clearing `requireOrganizerEligibility`'s automated checks goes live with zero human intervention (the only admin lever is the reactive force-cancel).
