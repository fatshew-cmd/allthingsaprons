...
**Profile page UI refactor:** Message button moved into a "More" dropdown (`···` icon button). Nominate button condensed to a compact icon (`boxicons:git-compare`). More menu includes: Direct Message, Share Profile, Block/Unblock, Report user — all fully wired.
...
**Signup flow refactored to 3 steps:** Account (username/email/password with real-time availability check via `/api/check-signup?email=`) → About You (display name/avatar/bio/sex/birthdate/marketing checkboxes) → Verify Email (OTP). Previously was 2 steps ("Profile Creation" and "Email Verification"). Step 1 and Step 2 each have independent validation; the confirmation toast + OTP send now fires from Step 2's Continue button. Back navigation added between steps.

**User block system:** `UserBlock` model (`blockerId`, `blockedId`, unique index). `POST /api/users/:username/block` toggles block on/off — returns `{ blocked: bool }`. On block, follow relationships in both directions are silently removed. Feed and explore exclude entries from blocked users (`$nin: blockedIds` added to entry query). Profile page computes `isBlocked` and passes it to the view — when blocked: Follow button hidden, DM link hidden, tab content replaced with "You've blocked this user" message. Block button in More menu is fully wired (fetch → toast → reload).

**User report system:** `UserReport` model (`reportedUserId`, `reportedBy`, `reasons[]`, `status: pending|approved|rejected`, unique index on `{reportedUserId, reportedBy}`). `POST /api/users/:username/report` — cannot self-report, deduplication enforced. Report button in profile More menu now opens a modal with multi-checkbox reasons: Spam or self-promotion, Harassment or bullying, Hate speech or discrimination, Impersonation, Underage user, Other. Admin moderation queue adds a "Users" tab (accessible via `?tab=users`) showing grouped reports per user with reason tags; `POST /admin/moderation/user-reports/:uid/dismiss` marks all pending reports for that user as `rejected`.

**Profile share tracking:** `ProfileShare` model (`sharerId`, `profileUserId`, `method: clipboard|native`) records share events. `ProfileShareView` model (`viewerId`, `profileUserId`, `referer`, `userAgent`) records when someone visits a profile via a share link. `POST /api/users/:username/share` fires on every share click and records the method. Share URLs generated by the More menu now include `?ref=share`; the profile page route creates a `ProfileShareView` doc whenever `?ref=share` is present in the query string. Share Profile in More menu is fully wired (native share API with clipboard fallback, toast confirmation).
...
| Contest eligibility gate | `utils/contestEligibility.js` — checks minEntries (3, lowered from 5 on 2026-07-01 for easier local testing), minRatingCount (25, lowered from 250 on 2026-07-01), minWeightedAvg (7.4, unchanged) before a user can create a contest or accept a nomination. Thresholds read from `PlatformSettings` collection. `TEST_BYPASS_USERNAMES` bypasses check for local dev accounts — kept intentionally for now, see `plans/July/july-action-plan.md` pre-launch reverts section. |
...
| Signup — 3-step flow | Refactored from 2 steps to 3: Account (username/email/password + real-time email availability check) → About You (display name/bio/sex/birthdate/checkboxes) → Verify Email (OTP). Back navigation between steps. |
| User block | `UserBlock` model. `POST /api/users/:username/block` toggle. On block: follow removed both ways. Feed/explore exclude blocked users' entries. Profile: Follow/DM hidden, content replaced with blocked message. Block/Unblock in More menu fully wired. |
...
- [ ] **REVERT BEFORE LAUNCH (on hold, not yet):** Re-enable the `status === 'active'` check in `routes/api.js` (contest vote handler) — commented out with `// TEMP` for local testing. Deliberately left disabled as of 2026-07-01 while testing is ongoing.
...
- [x] "Scrub forfeit record": `DELETE /contests/:id` extended to handle `void` contests. Caller must be a participant. Active contributions are refunded to contributors' `purchasedCHL` with `WalletTransaction` audit records, then the `Contest` and `Nomination` docs are deleted. Pending-case cancel path (creator only, no refund) unchanged.
...
- [x] **Announcement detail/stats page:** `GET /admin/announcements/:id` — fully live with real data across all metrics: impressions, clicks, CTR, dismiss rate. Stain field wired for affinity signals. Deletion cascades to all associated tracking docs. Activate/expire/delete actions included.
- [x] **All Entries browser:** `GET /admin/content` — paginated (50/page), searchable (title, stain, user), filterable by media type and visibility. Direct hide/unhide actions with automatic contest stalling/resumption and voting deadline extension. Gated behind `requireDomain('content')`.
...
#### Announcements — Stats Page (added June 18, implemented June 20, tracking wired June 30)
